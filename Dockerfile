...
# Install netskope-ms {{{
COPY --chown=1000:1000 . /opt/fmc_repository/CommandDefinition/netskope-ms
RUN install_repo_deps.sh /opt/fmc_repository/CommandDefinition/netskope-ms/

# Cleanup repository {{{
RUN rm -rf /opt/fmc_repository/CommandDefinition/netskope-ms/{.git,docker,Dockerfile}
# }}}
# Build tarball {{{
